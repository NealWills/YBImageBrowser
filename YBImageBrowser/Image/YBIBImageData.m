...
@implementation YBIBImageDataReDownloadUnit: NSObject
...
- (void)loadURL_download {
	...
	__weak typeof(self) wSelf = self;
	[[SDWebImageDownloader sharedDownloader] downloadImageWithURL:self.imageURL options:SDWebImageDownloaderLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
- (YBIBImageDataReDownloadUnit *)reDownloadUnit {
	if (!_reDownloadUnit) {
		_reDownloadUnit = [[YBIBImageDataReDownloadUnit alloc] init];
	}
	return _reDownloadUnit;
}
